...
public class CupiViajes implements Serializable
{
    ...
    {
        List<Hotel> lista = ( List<Hotel> )new java.awt.List( );
        Hotel resp = null;
        boolean encontro = false;

        Iterator<Hotel> i = hoteles.iterator( );
    ...
    {
        ReservaViaje resp = null;
        boolean encontro = false;
        Iterator<ReservaViaje> i = reservas.iterator( );

        while ( i.hasNext( ) && !encontro )
        {
            ReservaViaje temp = i.next( );
    ...
    public ReservaViaje buscarReservaMasPersonas( )
    {
        ...
        if ( reservas.isEmpty( ) )
        {
            ...
            ReservaViaje act = null;
            while ( i.hasNext( ) )
                act = i.next( );
            int cantActual = act.darCantidadAdultos( ) + act.darCantidadNinios( );
            {
    ...
    {
        ReservaViaje resp = null;
        int cantMenos = (reservas.get( 0 ).darCantidadAdultos( ) + reservas.get( 0 ).darCantidadAdultos( ));
        Iterator<ReservaViaje> i = reservas.iterator( );
        ...
        if ( reservas.isEmpty( ) )
        {
            ...
            int cantActual = act.darCantidadAdultos( ) + act.darCantidadNinios( );
            {
                if ( cantActual < cantMenos )
                {
                    cantMenos = cantActual;
                    resp = act;
                }
            }
        }
    ...
    {
        List<ReservaViaje> lista = ( List<ReservaViaje> )new java.awt.List( );
        Iterator<ReservaViaje> i = reservas.iterator( );
        
        while ( i.hasNext( ) )
        {
            ReservaViaje act = i.next( );
            if ( act.darAerolinea( ).equals( pAerolinea ) )
            {
                lista.add( act );
            }
        }
